...
const { User, UserActivity, Campaign, Item, Buyer, CampaignOperator, BrandSales, sequelize } = require('../models');
 ...
 * @desc    사용자 완전 삭제 (DB에서 모든 관련 데이터 삭제)
 * @access  Private (Admin only)
 */
router.delete('/:id', authenticate, authorize(['admin']), async (req, res) => {
  ...
  try {
    ...
    const { RefreshToken, UserActivity, UserMemo, Notification, SheetMemo, CampaignOperator } = require('../models');
    ...
    // 6. 브랜드-영업사 매핑 삭제 (영업사 또는 브랜드인 경우)
    ...
    // 7. 진행자 배정 삭제 (Operator인 경우)
    await CampaignOperator.destroy({
      where: { operator_id: userId },
      transaction
    });

    // 8. 다른 사용자의 assigned_sales_id를 null로 설정 (해당 영업사가 담당하던 브랜드들)
    await User.update(
    ...
    res.json({
      success: true,
      message: `사용자 "${user.name}" (${user.username})이(가) 완전히 삭제되었습니다.`
    });
  } catch (error) {
    await transaction.rollback();
