 ...
 */
exports.updateItemExpense = async (req, res) => {
  try {
    const { id } = req.params;
    const { expense_product, expense_courier, expense_review, expense_other, expense_note } = req.body;
    ...
    await item.update({
      expense_product: expense_product || null,
      expense_courier: expense_courier || null,
      expense_review: expense_review || null,
      expense_other: expense_other || null,
      expense_note: expense_note || null
    });

    res.json({
      success: true,
      message: '지출 정보가 저장되었습니다',
      data: item
    });
  } catch (error) {
    console.error('Update item expense error:', error);
    res.status(500).json({
      success: false,
      message: '지출 정보 저장 실패',
      error: error.message
 ...
 * Sales는 자신의 캠페인 품목만 조회
 */
exports.getMarginSummary = async (req, res) => {
  try {
    const { year_month, campaign_id } = req.query;
    const user = req.user;
    ...
    // Sales인 경우 자신의 캠페인만
    if (user.role === 'sales') {
      campaignWhereClause.created_by = user.id;
    }
      ...
      include: [
        {
          model: Campaign,
          as: 'campaign',
          where: Object.keys(campaignWhereClause).length > 0 ? campaignWhereClause : undefined,
          required: user.role === 'sales', // Sales인 경우 INNER JOIN
          attributes: ['id', 'name', 'created_by', 'registered_at'],
          include: [
            {
              model: User,
              as: 'creator',
    ...
    // 연월 필터 (year_month가 있으면 해당 월의 데이터만)
    let filteredItems = items;
    if (year_month) {
      filteredItems = items.filter(item => {
        const mb = item.campaign?.monthlyBrand;
        return mb && mb.year_month === year_month;
      });
    }
    ...
    const calculateItemMargin = (item) => {
      ...
      const courierRevenue = courierYn ? (courierPrice * totalCount) : 0;
      ...
      return {
        ...
        monthly_brand: item.campaign?.monthlyBrand?.name || '미분류',
        year_month: item.campaign?.monthlyBrand?.year_month,
        creator_name: item.campaign?.creator?.name,
    ...
    // 각 품목의 마진 계산
    const marginData = filteredItems.map(calculateItemMargin);

    // 총합 계산
    const summary = marginData.reduce((acc, item) => {
      acc.total_revenue += item.total_revenue;
      acc.total_revenue_vat += item.total_revenue_vat;
      acc.total_expense += item.total_expense;
      acc.total_margin += item.margin;
      return acc;
    }, { total_revenue: 0, total_revenue_vat: 0, total_expense: 0, total_margin: 0 });

    summary.margin_rate = summary.total_revenue_vat > 0
      ? (summary.total_margin / summary.total_revenue_vat * 100).toFixed(1)
      : 0;
    ...
    console.error('Get margin summary error:', error);
    res.status(500).json({
      success: false,
      message: '마진 조회 실패',
      error: error.message
 ...
 * 단일 품목 마진 조회
 */
exports.getItemMargin = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error('Get item margin error:', error);
    res.status(500).json({
      success: false,
      message: '마진 조회 실패',
      error: error.message
