...
### 물결표(`~`) 사용 금지
- 마크다운 렌더러에서 `~숫자~` 또는 `~텍스트~`가 **취소선**으로 보일 수 있음
- 사용자에게 "약/근사값/대략" 의미로 `~`를 쓰면 안 됨
- ❌ 잘못: `월 ~$100`, `약 ~7.5GB`, `5~10분`
- ✅ 올바름:
  - 단순 수치 범위: `5분 ~ 10분` (앞뒤로 공백 필수) 또는 그냥 `5분에서 10분`
  - 근사값: `약 7.5GB`, `대략 7.5GB`, `7.5GB 정도`
  - 비용: `월 80달러에서 150달러`, `월 80~150달러` (앞뒤 공백 필수)
- 안전한 방법: **물결표를 쓰지 말고 "약/대략/정도/에서/사이" 등 한글 단어로 표현**
...
- **한 구매자는 여러 이미지를 가질 수 있음** (재제출, 추가 업로드 등 정상 동작)
- **선 업로드(임시 Buyer)**: 구매자 정보 입력 전 먼저 업로드된 이미지. 이후 구매자 등록 시 병합 가능
...
### Bloggers (블로그 체험단 - Phase 1)
```
GET    /api/bloggers                       # 블로거 목록 (Admin: 전체+memo / Brand: 노출중인 것만)
POST   /api/bloggers                       # 블로거 등록 (Admin)
PUT    /api/bloggers/:id                   # 블로거 수정 (Admin)
DELETE /api/bloggers/:id                   # 블로거 삭제 (Admin, soft delete)
PATCH  /api/bloggers/:id/active            # 노출 토글 (Admin)
```

### Blogger Requests (발행 협의 요청 - Phase 2)
```
POST   /api/blogger-requests                          # 협의 요청 생성 (Brand, Admin)
GET    /api/blogger-requests/my                       # 내 요청 목록 (Brand, Admin) - viewAsUserId 지원
GET    /api/blogger-requests                          # 전체 인박스 (Admin, status 필터)
GET    /api/blogger-requests/:id                      # 요청 상세 (Admin, Brand 소유자)
PUT    /api/blogger-requests/:id                      # 요청 수정 (Admin)
PATCH  /api/blogger-requests/:id/cancel               # 요청 취소 (Brand 소유자, Admin)
PUT    /api/blogger-requests/items/:itemId            # 항목 수정 (Admin)
POST   /api/blogger-requests/items/:itemId/issue-token # 제출 토큰 발급 (Admin)
```

### Blogger Submit (작성 링크 제출 - Phase 3, Public)
```
GET    /api/blogger-submit/:token          # 제출 컨텍스트 조회 (Public)
POST   /api/blogger-submit/:token          # 작성 글 링크 제출 (Public)
```
...
- [x] **블로거(블로그 체험단) 중개 기능 도입 (Phase 1~4)**
  ...
  - **알림**: 기존 Notification ENUM 재사용(`campaign_created`/`target_reached`) + `reference_type='blogger_request'`로 구분 (ENUM 변경 회피).
  - **블로거는 시스템 로그인 없음** (공개 토큰 링크로만 제출). bloggers는 전역 공통(brand_id 없음), blogger_requests는 brand_id 소유권 필수.
  - 신규 마이그레이션 3개: `20260629000001~000003`. 배포 시 `npx sequelize-cli db:migrate` 필요.
