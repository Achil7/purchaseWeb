...
/**
 * 일별 제품 조회 (Sales/Operator 공용)
 * @param {string} role - 'sales' | 'operator'
 */
function MonthlyBuyerTable({ role = 'sales' }) {
  ...
  const loadItems = async () => {
    try {
      ...
      let response;
      if (role === 'sales') {
        response = await itemService.getItemsBySales();
      } else if (role === 'operator') {
        response = await itemService.getItemsByOperator();
      } else {
...
export default MonthlyBuyerTable;
