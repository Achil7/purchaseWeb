...
/**
 * 리뷰샷 검색 공통 대시보드 (Admin / Operator 양쪽에서 사용)
 *
 * Props:
 *  - viewAsUserId: number|null
 *      - operator wrapper에서 본인 ID 또는 admin view-operator의 대상 ID
 *      - 모든 백엔드 호출에 query 파라미터로 머지됨
 *      - 백엔드는 operator role일 경우 이 값을 무시하고 본인 ID 강제 (위조 방어)
 *  - scopeBrandFetcher: (viewAsUserId) => Promise — 브랜드사 옵션 fetcher
 *      - Admin 기본은 getBrandUsers(전체), Operator는 getBrandsForReviewSearch(배정 한정)
 *
 ...
 */
const ReviewSearchDashboard = ({ viewAsUserId = null, scopeBrandFetcher = getBrandUsers }) => {
  // 브랜드사 옵션
  const [brands, setBrands] = useState([]);
  const [brandsLoading, setBrandsLoading] = useState(false);
  const [noAssignment, setNoAssignment] = useState(false); // 배정 0건 상태
  ...
  // 브랜드사 목록 로드 (scopeBrandFetcher 주입으로 admin/operator 분기)
  useEffect(() => {
    (async () => {
      try {
        setBrandsLoading(true);
        const res = await scopeBrandFetcher(viewAsUserId);
        const list = Array.isArray(res) ? res : (res?.data || res?.users || []);
        setBrands(list);
        setNoAssignment(res?._scope === 'no_assignment');
      } catch (e) {
  ...
  }, [scopeBrandFetcher, viewAsUserId]);

  // viewAsUserId를 자동으로 머지하는 헬퍼
  const mergeViewAs = useCallback((params) => {
    if (viewAsUserId) return { ...params, viewAsUserId };
    return params;
  }, [viewAsUserId]);
  ...
  }, [selectedBrand, productName, startDate, endDate, accountHolder, platform, mergeViewAs]);
  ...
  const loadRange = useCallback(async (requestStart, requestEnd) => {
    ...
    await Promise.all(
      requests.map(async ({ offset, limit, key }) => {
        try {
          const res = await imageService.searchImages(mergeViewAs({
            ...currentQueryRef.current,
            limit,
            offset
          }));
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      {/* 배정 없음 안내 (operator/admin view-operator 모드에서 결과 0건 원인 구분) */}
      {noAssignment && (
        <Alert severity="info" sx={{ mb: 2 }}>
          배정된 캠페인이 없어 검색 가능한 데이터가 없습니다.
        </Alert>
      )}
...
export default ReviewSearchDashboard;
